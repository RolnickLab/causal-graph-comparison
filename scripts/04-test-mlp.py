...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # Target shape:  torch.Size([1, 1, 50, 1600])

            data, target = data.to(device), target.to(device)

            for step in range(FUTURE_TIMESTEPS):
                output = model(data.view(-1, INPUT_SIZE)) # output is of shape [1, 1600]

                output_list.append(output.cpu().numpy())
                data_list.append(data.cpu().numpy())
                target_list.append(target[0, 0, step].cpu().numpy())

                # shift data 1 to the left
                data = torch.roll(data, shifts=-1, dims=1)
                # set the last element to the output
                data[:, -1, :] = output.view(-1, LATITUDE * LONGITUDE)

                test_loss += F.mse_loss(output.view(-1), target[0, 0, step]).item()  # sum up batch loss

            break

    test_loss /= len(test_loader.dataset)

    data_array = np.array(data_list)[:, 0, :, 0, :]  # remove the 1s
    target_array = np.array(target_list)
    output_array = np.array(output_list)[:, 0, :]  # remove the 1s
